...
export default function LocalOfficerDashboard() {
  const { data: applications, isLoading, refetch } = useQuery({
    queryKey: ["local-officer-applications"],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("applications")
        .select("*")
        .in("status", ["submitted", "local_screening"])
  ...
  const reviewedApplications = applications?.filter(app => app.status === "local_screening") || [];
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      <div>
        <h1 className="text-3xl font-bold">Local Officer Dashboard</h1>
        <p className="text-muted-foreground">Review and approve ministerial applications from your area</p>
      </div>
              ...
              <Card key={app.id}>
                ...
                <CardContent>
                  <p className="text-sm text-muted-foreground">
                    Rejected on {new Date(app.local_reviewed_at || "").toLocaleDateString()}
                  </p>
